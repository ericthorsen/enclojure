...
(ns org.enclojure.editor.lex.ClojureLexer
  (:gen-class	
  ...
  (:require 
    [org.enclojure.editor.lex Lexer LexerInputReader]
...
;       :rdr (org.enclojure.nb.LexerInputReader. (.input info))}])
       :rdr (proxy [java.io.PushbackReader] [(org.enclojure.editor.lex.LexerInputReader. (.input info))]
	      (unread [ch] 
...
(defn -nextToken [this]  
   (let [{:keys [rdr factory input]} (.mstate this)]
     (try
      (let [{:keys [type token]} (org.enclojure.editor.lex.Lexer/get-token rdr)]
	(create-token factory 
		      token 
  ...
  (when-not *compile-files*
    (.language (proxy [org.netbeans.spi.lexer.LanguageHierarchy] []
		...
		;	      (sync nil (ref-set tokens nil))
			      (org.enclojure.editor.lex.ClojureLexer. info))
