...
It creates a logger using the namespace bound to *ns* at compile time."
  ([level msg]
    `(. #^Logger *logger* (~level #^String ~msg)))
  ([level fmt obj]
    `(. #^Logger *logger* (~level #^String ~fmt ~obj)))
  ([level fmt obj1 obj2]
    `(. #^Logger *logger* (~level #^String ~fmt ~obj1 ~obj2)))
...
(defmacro log-throwable
  ([level msg throwable]
    `(. #^Logger *logger* (~level #^String ~msg #^Throwable ~throwable))))

(defmacro def-logging-fn []
   (let [nsym# (str (ns-name *ns*))]
    `(def *logger* (LoggerFactory/getLogger ~nsym#))))
